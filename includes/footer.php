...
?>
    ...
    <?php endif; ?>
    <?php if (($caminho_atual ?? '') === '/profissional.php' && !empty($perfil['id'])): $online_perfil=usuario_esta_online($perfil);$resposta_media=tempo_medio_resposta_profissional((int)$perfil['id']); ?>
    <script>const titleProfile=document.querySelector('.public-profile aside h1');if(titleProfile){const status=document.createElement('div');status.className='presence-badge <?php echo $online_perfil?'online':'offline'; ?>';status.innerHTML='<i></i><?php echo sanitizar(texto_presenca($perfil)); ?>';titleProfile.after(status);const response=document.createElement('div');response.className='response-time';response.textContent=<?php echo json_encode($resposta_media?'Tempo médio de resposta: '.$resposta_media:'Tempo de resposta: ainda sem dados'); ?>;status.after(response)}</script>
    <?php endif; ?>
