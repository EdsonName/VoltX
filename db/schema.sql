...
CREATE TABLE anuncios_profissionais (id INT AUTO_INCREMENT PRIMARY KEY, codigo VARCHAR(24) UNIQUE, profissional_id INT NOT NULL, categoria VARCHAR(100) NOT NULL DEFAULT 'Geral', titulo VARCHAR(180) NOT NULL, texto VARCHAR(500), imagem_url VARCHAR(1000), midias_json LONGTEXT, video_url VARCHAR(1000), link_url VARCHAR(1000), duracao_segundos TINYINT UNSIGNED NOT NULL DEFAULT 10, status ENUM('rascunho','pendente','aprovado','rejeitado','encerrado') DEFAULT 'pendente', inicio_em DATETIME, fim_em DATETIME, criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(profissional_id) REFERENCES profissionais(id) ON DELETE CASCADE);
CREATE TABLE eventos_anuncios (id BIGINT AUTO_INCREMENT PRIMARY KEY, anuncio_id INT NOT NULL, usuario_id INT, tipo ENUM('impressao','clique','perfil') NOT NULL, criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(anuncio_id) REFERENCES anuncios_profissionais(id) ON DELETE CASCADE, FOREIGN KEY(usuario_id) REFERENCES usuarios(id) ON DELETE SET NULL, INDEX idx_eventos_anuncio_tipo(anuncio_id,tipo,criado_em));
CREATE TABLE visitas_perfis (id BIGINT AUTO_INCREMENT PRIMARY KEY, profissional_id INT NOT NULL, visitante_id INT NOT NULL, visitado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(profissional_id) REFERENCES profissionais(id) ON DELETE CASCADE, FOREIGN KEY(visitante_id) REFERENCES usuarios(id) ON DELETE CASCADE, INDEX idx_visitas_profissional(profissional_id,visitado_em));
