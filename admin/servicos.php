...
<div class="container">
    ...
    </form><aside class="service-preview"><span class="preview-label">Prévia do card</span><div class="preview-card"><div class="preview-image"><img src="<?php echo sanitizar($servico_edicao['imagem_url'] ?? ''); ?>" alt=""><span class="preview-placeholder">Imagem do serviço</span><b><?php echo sanitizar($servico_edicao['selo'] ?? 'Profissional'); ?></b></div><div class="preview-body"><h3>⚡ <span><?php echo sanitizar($servico_edicao['nome'] ?? 'Nome do serviço'); ?></span></h3><div class="price-row"><strong>A partir de R$ <span><?php echo isset($servico_edicao['preco']) && $servico_edicao['preco'] !== '' ? number_format((float)$servico_edicao['preco'],2,',','.') : '0,00'; ?></span></strong><span class="estimated-price-badge"><i></i> Valor estimado</span></div><p><?php echo sanitizar($servico_edicao['descricao'] ?? 'A descrição do serviço aparecerá aqui.'); ?></p><ul><li>✓ Benefício do serviço</li></ul><button type="button">Pedir no WhatsApp</button></div></div></aside></div>
    <?php endif; ?>
