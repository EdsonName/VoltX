...
$estilos_pagina = ['/assets/css/agendar.css?v=1'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$erros) {
        ...
        $stmt = $mysqli->prepare('INSERT INTO agendamentos (usuario_id, servico_id, data_horario, observacoes, cep, bairro_cidade, endereco, localizacao_gps) VALUES (?,?,?,?,?,?,?,?)');
        $stmt->bind_param('iissssss', $_SESSION['usuario_id'], $servico_id, $data_sql, $observacoes, $cep, $bairro_cidade, $endereco, $localizacao_gps);
        if ($stmt->execute()) {
